...
data_handler::~data_handler() {/*Free dynamically allocated data.*/ };
...
void data_handler::read_feature_vector(std::string path) {
	...
	if (f) { // If file read successfully.
		for (int i = 0; i < 4; i++) {
...
void data_handler::read_feature_labels(std::string path) {
	...
	if (f) {
		...
		std::cout << "Done getting label file." << std::endl;
		for (int i = 0; i < header[1]; i++) {
		...
		std::cout << "Successfully read and stored " << data_array->size() << " label vectors." << std::endl;
	} else {
		std::cout << "Could not find the file." << std::endl;
		exit(1);
	}
...
void data_handler::split_data() {
	...
	while (count < train_size) {
		...
		int rand_index = rand() % data_array->size(); // from 0 to data_array->size() - 1
		// If index can be used.
